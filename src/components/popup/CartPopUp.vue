<script setup>
    ...
    import PopUpEmbet from '@/components/popup/PopUpEmbet.vue';
...
<template>
    ...
    <PopUpEmbet @click="props.closeModal()"/>
    <div :class="['cart-pop-up bg-white w-96 h-full fixed right-0 top-0 z-20 p-8 flex flex-col', { 'is-open': props.openModal }]">
        <div class="cart__head flex justify-between items-center">
            <h4 class="text-xl font-bold">Корзина</h4>
                <ButtonItemIcon
                @click="props.closeModal()"
                :icon="icons.close"
                />
        </div>
        <div v-if="cartInfo.cartList.length > 0" class="h-full flex flex-col">
            <CartList :products="cartInfo.cartList"/>
            <div class="cart__footer mt-16 mb-16">
                <div class="flex mb-2">
                    <span>Всього:</span>
                    <span class="middle flex-auto border-b border-dashed relative -top-1"></span>
                    <span class="font-bold">
                        <span>{{ cartInfo.getTotalValue }}</span>
                        <span>грн.</span>
                    </span>
                </div>
        ...
        <div v-else class="m-auto">
            <svg fill="#000000" height="140px" width="140px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
